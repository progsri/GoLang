...
func main() {
	...
	fmt.Println("return of method 1 >> ", <-commonChannel1) //blocks until it has value.
	...
	fmt.Println("return of method 2 >> ", <-commonChannel2)
	...
	fmt.Println("return of method 3 >> ", <-commonChannel3)
...
func method2(channel2 <-chan string, channel3 chan<- string) {
	...
	compare := strings.Compare(valueFromChannel2, "method_1_proceed")
	fmt.Printf("MEthod2 .. comparing %d ", 0)
	for !(compare == 0) {
		//time.Sleep(10000)
